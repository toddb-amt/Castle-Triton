...
- [x] **ADM-05** · HIGH (POS product) · ✔ · `Fragment_page_admin_atm.java` — `requestNewWorkingKey()`, `testConnection()`, `downloadKeys()` each call `saveSettings()` before their host call; `saveSettings()` → `savePosSettings()` → `posConfig.setEnabled(cbEnablePosMode.isChecked())` — found during the 6.2.6 device pass (2026-09-18): POS mode silently OFF on the test unit — **in release/v6.2.6 `5dcfaee`**
