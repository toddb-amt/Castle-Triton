...
- [ ] **EMV-01** · LOW · ● observation 2026-09-25 — the chip amount (9F02) differs by path: walk-up sends **amount + fee** (`strAmount = total`), the POS gateway sends **amount only** (`strAmount = amountCents`). Both are approved by EFX (the issuer validates the ARQC against the 9F02 inside field 13, which is self-consistent). Decide which the processor expects (spec §EMV lists 9F02 "Amount, Authorized" without saying whether the surcharge is included) and make both paths agree. Not changed in 6.2.8.
